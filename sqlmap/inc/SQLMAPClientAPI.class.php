...
class SQLMAPClientAPI
{
  ...
  public function generateNewTaskID()
  {
    ...
    $json = json_decode(file_get_contents($this->api . "task/new", false, $context), true);
    ...
    if ((strtolower(trim($optstr)) != "evalcode") && (strtolower(trim($optstr)) != "eval")) {
      $opts = array(
        'http' => array(
          ...
          'content' => '{"url":"http://testphp.vulnweb.com/listproducts.php?cat=2"}',
          'timeout' => 60
        )
      );
      ...
      // echo "<br>";
      // var_dump("getOptionValue", $opts, $json);
      if ($json['success'] == "true") {
  ...
  */
  public function startScan($taskid)
  {
    ...
    // echo "<br>";
    // var_dump("startScan", $taskid, $opts, $json);
    if ($json['success'] == 1) {
